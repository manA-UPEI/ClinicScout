...
interface Props {
  kind: AgentErrorKind;
  message: string;
  /** Short id the server logged alongside this failure — shown so it can be quoted back for support. */
  requestId?: string;
  onRetry: () => void;
}
...
export default function ErrorState({ kind, message, requestId, onRetry }: Props) {
  return (
    <div
      className="animate-shake flex flex-col items-center gap-3 rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-950/20 p-8 text-center"
    >
      ...
      <button
        onClick={onRetry}
        className="mt-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition-all duration-150 hover:bg-blue-700 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98]"
      >
        Try another search
      </button>
    </div>
